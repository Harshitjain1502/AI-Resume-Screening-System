# 🎯 TalentPulse AI | Enterprise AI Resume Screening System

TalentPulse AI is a comprehensive, production-ready Applicant Tracking System (ATS) and talent intelligence platform. It automates manual candidate screening by leveraging advanced Natural Language Processing (NLP) and Machine Learning to parse, vectorize, evaluate, and rank candidate resumes against custom job specifications.

---

## 🚀 Key Features

*   **Premium Dark Cyber UI:** A modern, high-density dashboard built using Streamlit that replaces standard basic styling with professional container panels, live telemetry widgets, and glowing focus states.
*   **Dual-Engine Evaluation Framework:** Combines **Structural ML Suitability Predictions** (trained classifiers) with **Semantic Vector Alignment Scores** (TF-IDF Vectorization + Cosine Similarity) to calculate absolute candidate match percentages.
*   **Persistent PostgreSQL Architecture:** Structured database layer managed via SQLAlchemy ORM that synchronizes candidates, raw texts, predictions, and match logs dynamically.
*   **Multi-Format Parsing Pipeline:** Intelligent metadata harvesting engine that automatically strips and normalizes text, emails, and phone numbers out of attached `.pdf` and `.docx` portfolios.
*   **Recruiter Gatekeeper Security:** Built-in session state authentication barrier to shield internal applicant management assets from public access.
...
├── venv/                       # Local Python environment isolated dependencies
├── uploads/                    # Secure storage directory for candidate profile assets
├── models/                     # Saved binary files for trained inference models
├── app.py                      # Interactive Premium Dashboard application layout
├── database.py                 # Core database relational schema definition layer
├── train_model.py              # ML classifier optimization and training lifecycle 
├── ranking.py                  # Core text vectorization and matching algorithm
├── preprocess.py               # Document conversion, entity extraction, and regex tools
├── visualization.py            # Matplotlib data reporting plot generations
└── requirements.txt            # Operational package dependencies tree manifest
...
1. Position into Your Working Subdirectory
Bash
cd AI_Resume_Screening_System
2. Activate Virtual Environment & Sync Dependencies
Bash
# Activate existing environment
.\venv\Scripts\Activate

# Install project framework requirements
pip install -r requirements.txt
3. Initialize Database Tables
Bash
python database.py
4. Run the Machine Learning Training Script
Bash
python train_model.py
5. Launch the Enterprise Application Dashboard
Bash
streamlit run app.py
🔒 Administrative Portal Credentials
Username Identifier: admin

Security Access Token: recruiter2026